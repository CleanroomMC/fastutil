package com.cleanroommc.fastutil.shorts;
 ...
 */
public abstract class AbstractConcurrentShortList extends AbstractShortCollection implements ShortList, ShortStack {
    /** The locks. */
    private final ReadWriteLock lock = new ReentrantReadWriteLock();
    private final Lock wlock = lock.writeLock();
    private final Lock rlock = lock.readLock();

    protected AbstractConcurrentShortList() {
    }
    ...
    public ShortListIterator listIterator(final int index) {
        ...
        return new ShortIterators.AbstractIndexBasedListIterator(0, index) {
            ...
            protected short get(int i) {
                return AbstractConcurrentShortList.this.getShort(i);
            }

            @Override
            protected void add(int i, short k) {
                AbstractConcurrentShortList.this.add(i, k);
            }

            @Override
            protected void set(int i, short k) {
                AbstractConcurrentShortList.this.set(i, k);
            }

            @Override
            protected void remove(int i) {
                AbstractConcurrentShortList.this.removeShort(i);
            }

            @Override
            protected int getMaxPos() {
                return AbstractConcurrentShortList.this.size();
            }
        };
    }
    ...
    public ShortList subList(final int from, final int to) {
        ...
        try {
            ...
            return this instanceof java.util.RandomAccess ? new ConcurrentShortRandomAccessSubList(this, from, to) : new ConcurrentShortSubList(this, from, to);
        } finally {
            rlock.unlock();
        }
    }
     ...
     */
    public static class ConcurrentShortSubList extends AbstractConcurrentShortList implements Serializable {
        /** The locks. */
        private final ReadWriteLock lock = new ReentrantReadWriteLock();
        private final Lock wlock = lock.writeLock();
        private final Lock rlock = lock.readLock();
        ...
        public ConcurrentShortSubList(final ShortList l, final int from, final int to) {
            this.l = l;
            this.from = from;
            this.to = to;
        }
        ...
        private final class RandomAccessIter extends ShortIterators.AbstractIndexBasedListIterator {
            ...
            @Override
            protected void add(int i, short k) {
                ConcurrentShortSubList.this.add(i, k);
            }

            @Override
            protected void set(int i, short k) {
                ConcurrentShortSubList.this.set(i, k);
            }

            @Override
            protected void remove(int i) {
                ConcurrentShortSubList.this.removeShort(i);
            }
        ...
        private class ParentWrappingIter implements ShortListIterator {
            private final ShortListIterator parent;
        ...
        public ShortList subList(final int from, final int to) {
            ...
            return new ConcurrentShortSubList(this, from, to);
        }
    ...
    public static class ConcurrentShortRandomAccessSubList extends ConcurrentShortSubList implements RandomAccess {
        public ConcurrentShortRandomAccessSubList(final ShortList l, final int from, final int to) {
            super(l, from, to);
        }
        ...
        public ShortList subList(final int from, final int to) {
            ...
            // Sadly we have to rewrap this, because if there is a sublist of a sublist, and the
            // subsublist adds, both sublists need to update their "to" value.
            return new ConcurrentShortRandomAccessSubList(this, from, to);
        }
    }
}
