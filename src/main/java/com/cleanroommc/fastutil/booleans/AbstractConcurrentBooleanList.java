package com.cleanroommc.fastutil.booleans;
 ...
 */
public abstract class AbstractConcurrentBooleanList extends AbstractBooleanCollection implements BooleanList, BooleanStack {
    /** The locks. */
    private final ReadWriteLock lock = new ReentrantReadWriteLock();
    private final Lock wlock = lock.writeLock();
    private final Lock rlock = lock.readLock();

    protected AbstractConcurrentBooleanList() {
    }
    ...
    public BooleanListIterator listIterator(final int index) {
        ...
        return new BooleanIterators.AbstractIndexBasedListIterator(0, index) {
            ...
            protected boolean get(int i) {
                return AbstractConcurrentBooleanList.this.getBoolean(i);
            }

            @Override
            protected void add(int i, boolean k) {
                AbstractConcurrentBooleanList.this.add(i, k);
            }

            @Override
            protected void set(int i, boolean k) {
                AbstractConcurrentBooleanList.this.set(i, k);
            }

            @Override
            protected void remove(int i) {
                AbstractConcurrentBooleanList.this.removeBoolean(i);
            }

            @Override
            protected int getMaxPos() {
                return AbstractConcurrentBooleanList.this.size();
            }
        };
    }
    ...
    public BooleanList subList(final int from, final int to) {
        ...
        try {
            ...
            return this instanceof java.util.RandomAccess ? new ConcurrentBooleanRandomAccessSubList(this, from, to) : new ConcurrentBooleanSubList(this, from, to);
        } finally {
            rlock.unlock();
        }
    }
     ...
     */
    public static class ConcurrentBooleanSubList extends AbstractConcurrentBooleanList implements Serializable {
        /** The locks. */
        private final ReadWriteLock lock = new ReentrantReadWriteLock();
        private final Lock wlock = lock.writeLock();
        private final Lock rlock = lock.readLock();
        ...
        public ConcurrentBooleanSubList(final BooleanList l, final int from, final int to) {
            this.l = l;
            this.from = from;
            this.to = to;
        }
        ...
        private final class RandomAccessIter extends BooleanIterators.AbstractIndexBasedListIterator {
            ...
            @Override
            protected void add(int i, boolean k) {
                ConcurrentBooleanSubList.this.add(i, k);
            }

            @Override
            protected void set(int i, boolean k) {
                ConcurrentBooleanSubList.this.set(i, k);
            }

            @Override
            protected void remove(int i) {
                ConcurrentBooleanSubList.this.removeBoolean(i);
            }
        ...
        private class ParentWrappingIter implements BooleanListIterator {
            private final BooleanListIterator parent;
        ...
        public BooleanList subList(final int from, final int to) {
            ...
            return new ConcurrentBooleanSubList(this, from, to);
        }
    ...
    public static class ConcurrentBooleanRandomAccessSubList extends ConcurrentBooleanSubList implements RandomAccess {
        public ConcurrentBooleanRandomAccessSubList(final BooleanList l, final int from, final int to) {
            super(l, from, to);
        }
        ...
        public BooleanList subList(final int from, final int to) {
            ...
            // Sadly we have to rewrap this, because if there is a sublist of a sublist, and the
            // subsublist adds, both sublists need to update their "to" value.
            return new ConcurrentBooleanRandomAccessSubList(this, from, to);
        }
    }
}
