package com.cleanroommc.fastutil.chars;
 ...
 */
public abstract class AbstractConcurrentCharList extends AbstractCharCollection implements CharList, CharStack {
    /** The locks. */
    private final ReadWriteLock lock = new ReentrantReadWriteLock();
    private final Lock wlock = lock.writeLock();
    private final Lock rlock = lock.readLock();

    protected AbstractConcurrentCharList() {
    }
    ...
    public CharListIterator listIterator(final int index) {
        ...
        return new CharIterators.AbstractIndexBasedListIterator(0, index) {
            ...
            protected char get(int i) {
                return AbstractConcurrentCharList.this.getChar(i);
            }

            @Override
            protected void add(int i, char k) {
                AbstractConcurrentCharList.this.add(i, k);
            }

            @Override
            protected void set(int i, char k) {
                AbstractConcurrentCharList.this.set(i, k);
            }

            @Override
            protected void remove(int i) {
                AbstractConcurrentCharList.this.removeChar(i);
            }

            @Override
            protected int getMaxPos() {
                return AbstractConcurrentCharList.this.size();
            }
        };
    }
    ...
    public CharList subList(final int from, final int to) {
        ...
        try {
            ...
            return this instanceof java.util.RandomAccess ? new ConcurrentCharRandomAccessSubList(this, from, to) : new ConcurrentCharSubList(this, from, to);
        } finally {
            rlock.unlock();
        }
    }
     ...
     */
    public static class ConcurrentCharSubList extends AbstractConcurrentCharList implements Serializable {
        /** The locks. */
        private final ReadWriteLock lock = new ReentrantReadWriteLock();
        private final Lock wlock = lock.writeLock();
        private final Lock rlock = lock.readLock();
        ...
        public ConcurrentCharSubList(final CharList l, final int from, final int to) {
            this.l = l;
            this.from = from;
            this.to = to;
        }
        ...
        private final class RandomAccessIter extends CharIterators.AbstractIndexBasedListIterator {
            ...
            @Override
            protected void add(int i, char k) {
                ConcurrentCharSubList.this.add(i, k);
            }

            @Override
            protected void set(int i, char k) {
                ConcurrentCharSubList.this.set(i, k);
            }

            @Override
            protected void remove(int i) {
                ConcurrentCharSubList.this.removeChar(i);
            }
        ...
        private class ParentWrappingIter implements CharListIterator {
            private final CharListIterator parent;
        ...
        public CharList subList(final int from, final int to) {
            ...
            return new ConcurrentCharSubList(this, from, to);
        }
    ...
    public static class ConcurrentCharRandomAccessSubList extends ConcurrentCharSubList implements RandomAccess {
        public ConcurrentCharRandomAccessSubList(final CharList l, final int from, final int to) {
            super(l, from, to);
        }
        ...
        public CharList subList(final int from, final int to) {
            ...
            // Sadly we have to rewrap this, because if there is a sublist of a sublist, and the
            // subsublist adds, both sublists need to update their "to" value.
            return new ConcurrentCharRandomAccessSubList(this, from, to);
        }
    }
}
