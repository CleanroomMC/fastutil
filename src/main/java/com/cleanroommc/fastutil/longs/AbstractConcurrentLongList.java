package com.cleanroommc.fastutil.longs;
 ...
 */
public abstract class AbstractConcurrentLongList extends AbstractLongCollection implements LongList, LongStack {
    /** The locks. */
    private final ReadWriteLock lock = new ReentrantReadWriteLock();
    private final Lock wlock = lock.writeLock();
    private final Lock rlock = lock.readLock();

    protected AbstractConcurrentLongList() {
    }
    ...
    public LongListIterator listIterator(final int index) {
        ...
        return new LongIterators.AbstractIndexBasedListIterator(0, index) {
            ...
            protected long get(int i) {
                return AbstractConcurrentLongList.this.getLong(i);
            }

            @Override
            protected void add(int i, long k) {
                AbstractConcurrentLongList.this.add(i, k);
            }

            @Override
            protected void set(int i, long k) {
                AbstractConcurrentLongList.this.set(i, k);
            }

            @Override
            protected void remove(int i) {
                AbstractConcurrentLongList.this.removeLong(i);
            }

            @Override
            protected int getMaxPos() {
                return AbstractConcurrentLongList.this.size();
            }
        };
    }
    ...
    public LongList subList(final int from, final int to) {
        ...
        try {
            ...
            return this instanceof java.util.RandomAccess ? new ConcurrentLongRandomAccessSubList(this, from, to) : new ConcurrentLongSubList(this, from, to);
        } finally {
            rlock.unlock();
        }
    }
     ...
     */
    public static class ConcurrentLongSubList extends AbstractConcurrentLongList implements Serializable {
        /** The locks. */
        private final ReadWriteLock lock = new ReentrantReadWriteLock();
        private final Lock wlock = lock.writeLock();
        private final Lock rlock = lock.readLock();
        ...
        public ConcurrentLongSubList(final LongList l, final int from, final int to) {
            this.l = l;
            this.from = from;
            this.to = to;
        }
        ...
        private final class RandomAccessIter extends LongIterators.AbstractIndexBasedListIterator {
            ...
            @Override
            protected void add(int i, long k) {
                ConcurrentLongSubList.this.add(i, k);
            }

            @Override
            protected void set(int i, long k) {
                ConcurrentLongSubList.this.set(i, k);
            }

            @Override
            protected void remove(int i) {
                ConcurrentLongSubList.this.removeLong(i);
            }
        ...
        private class ParentWrappingIter implements LongListIterator {
            private final LongListIterator parent;
        ...
        public LongList subList(final int from, final int to) {
            ...
            return new ConcurrentLongSubList(this, from, to);
        }
    ...
    public static class ConcurrentLongRandomAccessSubList extends ConcurrentLongSubList implements RandomAccess {
        public ConcurrentLongRandomAccessSubList(final LongList l, final int from, final int to) {
            super(l, from, to);
        }
        ...
        public LongList subList(final int from, final int to) {
            ...
            // Sadly we have to rewrap this, because if there is a sublist of a sublist, and the
            // subsublist adds, both sublists need to update their "to" value.
            return new ConcurrentLongRandomAccessSubList(this, from, to);
        }
    }
}
