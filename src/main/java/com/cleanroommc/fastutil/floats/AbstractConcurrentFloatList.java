package com.cleanroommc.fastutil.floats;
 ...
 */
public abstract class AbstractConcurrentFloatList extends AbstractFloatCollection implements FloatList, FloatStack {
    /** The locks. */
    private final ReadWriteLock lock = new ReentrantReadWriteLock();
    private final Lock wlock = lock.writeLock();
    private final Lock rlock = lock.readLock();

    protected AbstractConcurrentFloatList() {
    }
    ...
    public FloatListIterator listIterator(final int index) {
        ...
        return new FloatIterators.AbstractIndexBasedListIterator(0, index) {
            ...
            protected float get(int i) {
                return AbstractConcurrentFloatList.this.getFloat(i);
            }

            @Override
            protected void add(int i, float k) {
                AbstractConcurrentFloatList.this.add(i, k);
            }

            @Override
            protected void set(int i, float k) {
                AbstractConcurrentFloatList.this.set(i, k);
            }

            @Override
            protected void remove(int i) {
                AbstractConcurrentFloatList.this.removeFloat(i);
            }

            @Override
            protected int getMaxPos() {
                return AbstractConcurrentFloatList.this.size();
            }
        };
    }
    ...
    public FloatList subList(final int from, final int to) {
        ...
        try {
            ...
            return this instanceof java.util.RandomAccess ? new ConcurrentFloatRandomAccessSubList(this, from, to) : new ConcurrentFloatSubList(this, from, to);
        } finally {
            rlock.unlock();
        }
    }
     ...
     */
    public static class ConcurrentFloatSubList extends AbstractConcurrentFloatList implements Serializable {
        /** The locks. */
        private final ReadWriteLock lock = new ReentrantReadWriteLock();
        private final Lock wlock = lock.writeLock();
        private final Lock rlock = lock.readLock();
        ...
        public ConcurrentFloatSubList(final FloatList l, final int from, final int to) {
            this.l = l;
            this.from = from;
            this.to = to;
        }
        ...
        private final class RandomAccessIter extends FloatIterators.AbstractIndexBasedListIterator {
            ...
            @Override
            protected void add(int i, float k) {
                ConcurrentFloatSubList.this.add(i, k);
            }

            @Override
            protected void set(int i, float k) {
                ConcurrentFloatSubList.this.set(i, k);
            }

            @Override
            protected void remove(int i) {
                ConcurrentFloatSubList.this.removeFloat(i);
            }
        ...
        private class ParentWrappingIter implements FloatListIterator {
            private final FloatListIterator parent;
        ...
        public FloatList subList(final int from, final int to) {
            ...
            return new ConcurrentFloatSubList(this, from, to);
        }
    ...
    public static class ConcurrentFloatRandomAccessSubList extends ConcurrentFloatSubList implements RandomAccess {
        public ConcurrentFloatRandomAccessSubList(final FloatList l, final int from, final int to) {
            super(l, from, to);
        }
        ...
        public FloatList subList(final int from, final int to) {
            ...
            // Sadly we have to rewrap this, because if there is a sublist of a sublist, and the
            // subsublist adds, both sublists need to update their "to" value.
            return new ConcurrentFloatRandomAccessSubList(this, from, to);
        }
    }
}
